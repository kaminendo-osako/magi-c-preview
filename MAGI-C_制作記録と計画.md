...
### Phase 2-c — 通知（メール／LINE）← **必ず実施（決済の通し後すぐ）**
> ⚠ **忘れないこと**（依頼者の明示要望）。webhook 基盤ができているので追加は容易。
- **予約確定メール**（お客様＋オーナー）：`stripe-webhook` の確定処理内、または `confirm_booking_paid` 後に送信。送信は **Resend / SendGrid 等をEdge Functionから**（APIキーはSupabase Secrets）。
- **LINE通知**：⚠ **LINE Notifyは終了済み → LINE Messaging API** を使う（[5-7](#5-7-技術セキュリティの禁止事項)）。チャネルトークンは**Supabase Secrets**、フロント・Next.js側に直書きしない。
