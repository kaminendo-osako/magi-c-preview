...
- **人間側の有効化手順（テスト）**：
  1. **Resendで送信サブドメイン `send.kaminendo.art` を検証**（Add Domain→表示DNS 3種[SPF/DKIM/Return-Path]をお名前.comへ→Verify緑）
  ...
  5. Secrets：`supabase secrets set RESEND_API_KEY=re_xxx` ／ `supabase secrets set SHARED_MAIL_DOMAIN=send.kaminendo.art`
