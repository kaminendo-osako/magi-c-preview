...
#### 2-c-1 メール通知 ← **コード実装済み（2026-07-19）。Resendキー設定で有効化**
- **仕組み**：決済確定（`checkout.session.completed`）→ `confirm_booking_paid` が予約詳細を返す →
  `stripe-webhook` が `sendBookingConfirmation()` で **お客様＋オーナー** に確定メール送信。
  メール失敗は握りつぶして webhook は 200（Stripe再送ループ回避）。金額・氏名は確定 booking から取得（フロント非経由）。
- **成果物**：`functions/_shared/notify.ts`（Resend送信・HTMLエスケープ）／`schema-notifications.sql`
  （`confirm_booking_paid` を確定＋詳細返却に拡張・service_role限定）／`stripe-webhook` 更新。
- **非破壊**：`RESEND_API_KEY` 未設定なら送信せず素通り（決済フローは通常どおり）。
- **人間側の有効化手順（テスト）**：
  1. [Resend](https://resend.com) 無料登録 → API Keys で `re_...` を取得
  2. **SQL Editor で `schema-notifications.sql` を実行**（`confirm_booking_paid` を差し替え）
  3. `cd booking-product` → `supabase functions deploy stripe-webhook`
  4. Secrets 設定：
     ```bash
     supabase secrets set RESEND_API_KEY=re_xxx
     supabase secrets set OWNER_NOTIFICATION_EMAIL=オーナーの受信先@example.com
     # 送信元。独自ドメイン未検証のうちは省略可（Resendテスト送信元にフォールバック）
     supabase secrets set NOTIFICATION_FROM="MAGI-C <yoyaku@bebest-magic.com>"
     ```
  5. テスト予約→決済→**お客様・オーナー双方にメール**が届くか確認 → テスト予約は削除
- **⚠ Resendの制約**：独自ドメイン未検証のうちは「自分のResend登録メール宛」しか送れない。本番は
  **`bebest-magic.com` をResendで検証**（お名前.comにDNSレコード追加）し、`yoyaku@bebest-magic.com` 等から送る。
- **本番送信の前提**：特商法・プライバシーポリシー掲示（本番公開前タスクと合流）。
