...
| `rental.html` | レンタル予約（4ステップ説明＋事前決済の注記＋ウィジェット設置枠） | 骨組み完成・**仮画像挿入済み** |
...
> Phase 2-a（決済なしの予約保存）は **設計・実装済み**。「2-a から始める」方針は確定済みのため、論点は「接続の実行」と「実写真の準備」に移っている。
1. **Supabase の用意＋接続**（Phase 2-a を本番化）：Tokyoリージョンでプロジェクト作成 → `schema.sql` を実行 → **Project URL** と **anon public** キーの2値を共有 → `supabase-config.js` に貼って接続後、実予約を一緒に検証
2. **Stripe アカウントの用意**（Phase 2-b 用。審査に時間がかかるため早めに着手推奨）
3. **本番写真の撮影・実写真への差し替え準備**（現在は仮画像。撮影方針は [5-2](#5-2-写真の禁止事項と方針)／[8](#8-写真仮画像手書き風水彩風の方針) 参照）
