...
また、現在の成果物は **写真ゼロのデザイン骨組み（プロトタイプ）** であり、まだ「完成」ではない。
写真・実コピー・空間の質感が入って初めて、MAGI-Cの「高級感×さわやかさ」は成立する。
...
| ファイル | 役割 | 状態 |
|---|---|---|
| `index.html` | トップ（世界観／各機能への導線／姉妹店／アクセス／LINE CTA） | 骨組み完成・**写真未挿入** |
| `facilities.html` | 設備一覧（6カード＋料金表。料金表は横スクロール対応済み） | 骨組み完成・**写真未挿入** |
| `rental.html` | レンタル予約（4ステップ説明＋事前決済の注記＋ウィジェット設置枠） | 骨組み完成 |
| `cafe.html` | カフェ予約（営業カレンダー＋LINE誘導。Amelia不使用） | 骨組み完成 |
| `contact.html` | お問い合わせフォーム（送信未接続のプレースホルダ）＋LINE | 骨組み完成 |
| `access.html` | アクセス（情報リスト＋Google Maps iframe） | 骨組み完成 |
...
### Phase 2-a — 予約データの保存＋管理画面（決済なし）← **設計・実装ほぼ完了。あとは接続待ち**
...
1. Phase 2 をどちらから始めるか（推奨：**決済を絡めない 2-a から**）
2. Supabase アカウントの用意
3. Stripe アカウントの用意（審査に時間がかかるため早めに着手推奨）
4. **写真素材の方針決め**（[5-2](#5-2-写真の禁止事項と方針) 参照）
...
### 8-2. 仮画像一覧（区分：必須／推奨／任意）
| ファイル名 | ページ・場所 | 比率 | 区分 | alt案（実写差し替え後もそのまま使える自然な説明） |
|---|---|---|---|---|
| `home-hero.jpg` | index：Hero背景（`.hero__media`） | 21:9（最低16:9） | 必須 | 装飾背景（H1が意味を担うためaltなし可） |
| `home-concept.jpg` | index：Concept節（`.media-frame`） | 4:3 | 必須 | 窓辺の木のテーブルと観葉植物のある、落ち着いた店内の様子 |
| `cafe-scene.jpg` | cafe：左カラム上部 | 4:3 | 必須 | 窓辺のテーブルに置かれた一杯のコーヒー |
| `ogp.png` | 全ページ`<head>`（まずindex） | 1200×630 | 必須 | OGP用（alt不要） |
| `home-sister.jpg` | index：Sister節（`.media-frame`） | 4:3 | 推奨 | 緑のある入口がやさしい雰囲気の姉妹店の外観イメージ |
| `facility-multipurpose.jpg` | facilities：カード01 | 3:2 | 推奨 | 自然光の入る多目的スペース |
| `facility-kitchen.jpg` | facilities：カード02 | 3:2 | 推奨 | 明るく清潔なキッチン設備 |
| `facility-workspace.jpg` | facilities：カード03 | 3:2 | 推奨 | 電源のある静かな作業コーナー |
| `facility-av.jpg` | facilities：カード04 | 3:2 | 推奨 | プロジェクターと音響のある部屋 |
| `facility-furniture.jpg` | facilities：カード05 | 3:2 | 推奨 | 整えられたテーブルと椅子 |
| `facility-amenity.jpg` | facilities：カード06 | 3:2 | 推奨 | ドリンクサービスのアメニティコーナー |
| `pagehero-band.jpg` | 下層ページ`.page-hero`背景（薄く） | 21:9 | 任意 | 装飾背景 |
| `rental-space.jpg` | rental：ステップ上部 | 4:3 | 任意 | 貸切・教室などレンタル利用の様子 |
...
- 拡張子も統一（当面 `.jpg`、OGPは `.png`）。WebP化・`srcset`は最終最適化フェーズで。
...
7. 本ドキュメントの「写真未挿入」ステータスを更新。

### 8-7. 既存デザインを壊さない実装方針（実装は承認後）
