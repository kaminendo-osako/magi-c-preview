...
### Phase 2-b — Stripe Checkout 接続（レンタルの事前決済）← **テストモードで通し検証済み（2026-07-19）**

> **検証結果（2026-07-19・実データ）**：ローカルのウィジェット→Edge Function→Stripe Checkout（テスト）→カード `4242…`→ `booking-success.html` で「確定」＋予約番号表示。webhook で `pending_payment→confirmed`。同枠の再予約は**確定枠・仮押さえ枠のいずれも `slot_taken` で拒否**、空き枠は Checkout URL 発行。空き状況は過去日/定休/予約を正しく反映。**残る人間作業：診断コード撤去後の再デプロイ・テストデータ削除・push・本番有効化（Stripe activation）は本番公開時。**

> ⚠️ **新API keys 対応で必要だった修正（再販先でも効く／`schema-payments.sql`・`_shared/db.ts` に反映済み）**
> 1. **Secret keyは `apikey` ヘッダーのみで送る**：新形式 Secret key（`sb_secret_`）を `Authorization: Bearer` に載せると JWT 解析失敗で権限のないロールに落ちる。`_shared/db.ts` は `apikey` のみで REST を叩く（`createClient` を使わない）。環境変数は `SUPABASE_SECRET_KEYS`（JSON）または後方互換の `SUPABASE_SERVICE_ROLE_KEY`。
> 2. **`service_role` に master テーブルの GRANT が必要**：`service_role` は RLS は回避するが GRANT は別。新プロジェクト既定では未付与のことがあり、`grant select on tenant/space/time_slot/plan/closed_weekday to service_role` を `schema-payments.sql` に追記。
- レンタルのみ事前決済。**Stripe Checkout（ホスト型）でPCI範囲を最小化**
- **方式B（仮押さえ→確定）**：決済完了まで `confirmed` を作らない。仮押さえ45分・Checkout失効32分。
- **秘密情報はすべて Supabase 側**（Stripe Secret / Webhook secret / service_role）。フロントは Publishable key のみ・金額も送らない（DBの `plan.price_yen`）。
- **WordPress化しても不変**：予約は独立プロダクトのまま iframe 埋め込み。切替は `supabase-config.js` の `enableStripe` と Edge Function の `ALLOWED_RETURN_ORIGINS` に本番ドメインを足すだけ。

#### 実装済み成果物（2026-07-03）
| ファイル | 役割 |
|---|---|
| `booking-product/supabase/schema-payments.sql` | booking拡張（`pending_payment`/`expired`/`expires_at`/`stripe_session_id`/`paid_at`/日本語`status_label`生成列）＋UNIQUE索引を「確定＋仮押さえ」に拡大＋空き状況関数を仮押さえ考慮に更新＋RPC（`create_pending_booking`/`attach_checkout_session`/`confirm_booking_paid`/`expire_booking`/`release_expired_holds`/`booking_public_status`）＋管理ビュー`booking_admin_ja`。決済系RPCは**service_role限定**（anon遮断） |
| `booking-product/supabase/functions/create-checkout-session/` | 予約内容を受け取り→DB側で再検証・仮押さえ→Stripe Checkout作成→URL返却。戻りURLは許可リスト照合 |
| `booking-product/supabase/functions/stripe-webhook/` | **署名検証必須**。`completed`→確定／`expired`→期限切れ。冪等 |
| `booking-product/supabase/config.toml` | 両Functionを`verify_jwt=false` |
| `booking-success.html` / `booking-cancel.html` | 決済後の戻り先（独立ページ・noindex・テスト注記つき） |
| `widget.js` / `supabase-config.js` | `enableStripe`（既定false＝現行「予約のみ」のまま非破壊）。trueでカード決済→Checkout |

#### 人間側の作業（この順で・テストモード）
> **`enableStripe` は現在 false**（デプロイ前にtrueにすると決済失敗するため）。下記5まで済んだら true にして検証→push。

1. **Stripeアカウント作成**（テストモードは審査前でも使用可）→ ダッシュボードを**テストモード**にし、開発者→APIキーで **`sk_test_...`（Secret key）** を控える
2. **Supabase CLI 導入＋リンク**（運用が楽）：
   ```bash
   npm i -g supabase        # または scoop/brew
   supabase login
   cd booking-product            # ./supabase/ を含むディレクトリで実行する
   supabase link --project-ref yudabqfivpgfiqxozccu
   ```
3. **DBに決済スキーマを反映**：SQL Editor で `schema-payments.sql` を全文実行（冪等）
4. **Secrets 設定＋デプロイ**：
   ```bash
   supabase secrets set STRIPE_SECRET_KEY=sk_test_xxx
   supabase secrets set ALLOWED_RETURN_ORIGINS="https://kaminendo-osako.github.io,http://localhost:5500"
   supabase functions deploy create-checkout-session
   supabase functions deploy stripe-webhook
   ```
5. **Webhook 登録**：Stripe→開発者→Webhook→エンドポイント追加
   `https://yudabqfivpgfiqxozccu.supabase.co/functions/v1/stripe-webhook`
   イベント：`checkout.session.completed`・`checkout.session.expired` → 表示される **`whsec_...`** を
   `supabase secrets set STRIPE_WEBHOOK_SECRET=whsec_xxx`
6. **切替＆検証**：`supabase-config.js` の `enableStripe: true` → ローカル(`localhost:5500`)で通し：予約→Checkout→**テストカード `4242 4242 4242 4242`（有効期限は未来・CVC任意）**→ success戻り→ Studioで`確定`確認／同枠の再予約が弾かれる／キャンセルで`booking-cancel`戻り。OKなら push（Pagesにも反映）。

#### テスト・本番モードの切替
- テスト↔本番は **Secrets のキーを差し替えるだけ**（`sk_test_`↔`sk_live_`、本番Webhookの`whsec_`）。フロント変更なし。
- **本番開始の前提**：特商法表記・キャンセルポリシー・プライバシーポリシーの掲示（本番公開前タスク）。本番決済は通し検証後に別途判断。

#### Supabase Studio での見え方（オーナーも操作可）
- `booking` テーブルに **`status_label`**（決済待ち／確定／キャンセル／期限切れ）列が出る。
- 読みやすい一覧は **ビュー `booking_admin_ja`**（予約日・時間枠・プラン・氏名・金額・ステータス・決済日時 等）。Table Editor か SQL Editor（`select * from booking_admin_ja;`）で確認。

### Phase 2-c — 通知（メール／LINE）
> ⚠ 依頼者の明示要望。webhook 基盤ができているので追加は容易。

#### 2-c-1 メール通知（マルチテナント対応）← **Phase A 実装済み（2026-07-19）。ドメイン検証＋Secretsで有効化**
> 分析（マルチテナントSaaS化）に基づく**二層構造**。詳細設計は別途 §2-c-arch（下）。

- **アーキテクチャ**：From は常に**製品が認証した共有ドメイン**（`SHARED_MAIL_DOMAIN`＝`send.kaminendo.art`）から。テナント個性は **From表示名＝屋号／Reply-To＝オーナー宛**で表現（＝テナントのDNS作業ゼロ）。希望テナントのみ自ドメイン検証（`sending_mode='custom_domain'`）へ**無停止昇格**。任意アドレスFromは不可（DMARC failのため）。
- **仕組み**：決済確定 → `confirm_booking_paid` が **確定＋送信ペイロード（テナント設定込み）＋`confirmation_sent_at`** を返す → `stripe-webhook` が**未送のときだけ** `sendBookingConfirmation()` を実行 → 成功で `mark_confirmation_sent`／失敗で `record_confirmation_error`＋500返却（Stripe再送・冪等キーで二重送信防止）。**送信状態を決済確定から分離**＝Resend一時失敗でも永久欠落しない（辛口レビュー欠陥①の修正）。
- **成果物**：`schema-notifications.sql`（`tenant_notification_settings`／`email_suppression`／`booking.confirmation_sent_at`ほか／`confirm_booking_paid`拡張／`mark_confirmation_sent`・`record_confirmation_error`・全て service_role 限定・MAGI-C行シード）／`functions/_shared/email/provider.ts`（プラガブル `EmailProvider`＝Resend実装・将来SES差替え可）／`functions/_shared/notify.ts`（テナント設定でFrom組立・**ヘッダインジェクション対策のサニタイズ**・冪等キー・サプレッション対応）／`stripe-webhook` 更新。
- **非破壊**：`RESEND_API_KEY` or `SHARED_MAIL_DOMAIN` 未設定なら無送信で素通り（決済フローは通常どおり）。
- **人間側の有効化手順（テスト）**：
  1. **共有送信ドメイン**：Resendの**既に検証済みドメインを再利用**（新規追加・Pro不要。無料枠の1ドメインで全テナントをまかなえる＝Layer1の利点）。現状 `mail.shusei-r-system.kaminendo.art`(Verified)を使用。
  2. **SQL Editor で `schema-notifications.sql` を実行**（テーブル追加＋`confirm_booking_paid`差し替え＋MAGI-C行シード）
  3. **MAGI-Cの通知先を設定**：`update tenant_notification_settings set reply_to_email='クライアントのメール', owner_notify_email='クライアントのメール' where tenant_id=(select id from tenant where slug='magic');`
  4. `cd booking-product` → `supabase functions deploy stripe-webhook`
  5. Secrets：`supabase secrets set RESEND_API_KEY=re_xxx` ／ `supabase secrets set SHARED_MAIL_DOMAIN=mail.shusei-r-system.kaminendo.art`
  6. テスト予約→決済→**お客様（予約時のメール）・オーナー（設定した通知先）双方にメール**確認 → テスト予約削除
  > 💰 **コスト方針**：無料枠=1ドメインで足りる（共有ドメイン方式）。Pro($20/mo・10ドメイン)は「自ドメイン送信(Layer2)を望む**有料テナント**が複数出てから」＝売上に追従。先行課金しない。
- **Phase B（堅牢化）**：`resend-webhook`（svix署名・バウンス/苦情→`email_suppression`＋`email_event`）／苦情率監視（二段判定）／送信失敗の定期リトライ（cron）。
- **本番送信の前提**：本文にテナント事業者情報（特商法）明記／プライバシーポリシーにResend(US)越境移転を記載。

#### 2-c-2 LINE通知 ← **未実装（次段）**
- ⚠ **LINE Notifyは終了済み → LINE Messaging API** を使う（[5-7](#5-7-技術セキュリティの禁止事項)）。
- チャネルアクセストークンは **Supabase Secrets**、フロント・Next.js側に直書きしない。
- オーナー通知には LINE公式アカウント＋オーナーの userId（or 通知グループ）が必要。LINE公式の準備状況を 7/6 で確認してから着手。
- 決済なし（現地決済）テナントの「予約のみ」経路にも通知を付けられる設計にする。
...
3. **Stripe接続の実施（Phase 2-b）**：コードは実装済み。人間側の手順は上記「人間側の作業（この順で・テストモード）」を実行（Stripeアカウント→CLI→schema-payments.sql→Secrets＆deploy→Webhook→enableStripe:true→通し検証）
4. **決済の通し後：Phase 2-c 通知（メール／LINE Messaging API）を必ず実施**（依頼者の明示要望）
5. **2026-07-06 依頼者ヒアリング**：文言・写真の方向性・料金や住所等の実値・LINE公式の準備状況を確認
...
**Phase 2-b 決済テストで作ったデータを一括削除（2026-07-19 の検証後クリーンアップ）：**
```sql
delete from booking
where customer_email in ('test@example.com','test-flow@example.com','dup@example.com')
   or customer_name in ('テスト太郎','テスト花子','二重予約テスト','テスト二郎');
-- 未確定の仮押さえ（期限切れ含む）をまとめて掃除したい場合：
-- delete from booking where status in ('pending_payment','expired');
```
